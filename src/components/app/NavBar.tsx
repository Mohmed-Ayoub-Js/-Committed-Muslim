...
import { Book, GraduationCap, HomeIcon, MessageCircle, Bookmark } from 'lucide-react';
...
const NavBar = () => {
  ...
  return (
    <div className="fixed flex items-center flex-col">
      <img src='icon.webp' className='w-[50px] h-[50px] hover:w-[100px] hover:h-[100px] cursor-pointer' style={{transition:'0.5s'}} alt="Icon" />
      {data.map((item, index) => (
        <div key={index}>
          <Button variant="outline" className="w-[200px] mt-2 mb-2 p-3 hover:bg-black hover:text-white flex justify-between items-center flex-row">
            <p>{item.icon}</p>
            {item.title}
          </Button>
        </div>
      ...
      <div>
        <Card className={`w-[200px]  cursor-pointer ${isFlashing ? 'bg-black text-white' : ''}`} style={{ transition: '0.5s' }}>
          <CardHeader>
